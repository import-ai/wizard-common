...
- Each retrieval starts with <cite ...> and ends with </cite>. If the retrieval has a `cite_marker` attribute, that is the exact citation marker for that result.
- Please copy exact `cite_marker` values such as `[[1]](C1-water-temperature-L12-18)` to cite your sources.
- If a sentence comes from multiple search results, list all applicable `cite_marker` values.
...
- If there is no `cite_marker` contained in the search result, do not fabricate one.
